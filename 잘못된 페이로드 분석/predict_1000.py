...
# 데이터 로드
file_path = '../flow_file/All_random_1000.csv'

if not os.path.exists('./roberta/trained_model_12_1000/matrixs'):
    os.makedirs('./roberta/trained_model_12_1000/matrixs')
if not os.path.exists('./roberta/trained_model_12_1000/indexs'):
    os.makedirs('./roberta/trained_model_12_1000/indexs')
if not os.path.exists('./roberta/trained_model_12_1000/results'):
    os.makedirs('./roberta/trained_model_12_1000/results')
if not os.path.exists('./roberta/trained_model_12_1000/logs'):
    os.makedirs('./roberta/trained_model_12_1000/logs')
...
for train_index, test_index in kf.split(tokenized_data):
    ...
    training_args = TrainingArguments(
        output_dir=f'./roberta/trained_model_12_1000/results/fold_{fold}', num_train_epochs=3, per_device_train_batch_size=4, per_device_eval_batch_size=8,
        gradient_accumulation_steps=8, gradient_checkpointing=False, warmup_steps=600, weight_decay=0.01,
        logging_dir=f'./roberta/trained_model_12_1000/logs/fold_{fold}', logging_steps=10, evaluation_strategy="epoch", save_strategy="epoch", load_best_model_at_end=True,
    ...
    # 모델 학습
    trainer.train()
    model.save_pretrained(f'./roberta/trained_model_12_1000/model/fold_{fold}')
    ...
    # 결과 저장 및 출력
    app_metrics_df.to_csv(f'./roberta/trained_model_12_1000/results/fold_{fold}_application_metrics.csv', index=False)
    ...
    plt.tight_layout()
    plt.savefig(f'./roberta/trained_model_12_1000/matrixs/fold_{fold}_confusion_matrix.png')
    plt.close()

    # 예측 결과 저장 (추가된 부분)
    results_path = f'./roberta/trained_model_12_1000/results/fold_{fold}_predictions.csv'
    results_df = pd.DataFrame({'true_labels': true_labels, 'pred_labels': pred_labels})
    ...
    # 훈련, 검증, 테스트 데이터 저장
    save_data(f'./roberta/trained_model_12_1000/indexs/fold_{fold}_train_data.csv', train_inputs, train_masks, train_labels)
    save_data(f'./roberta/trained_model_12_1000/indexs/fold_{fold}_val_data.csv', val_inputs, val_masks, val_labels)
    save_data(f'./roberta/trained_model_12_1000/indexs/fold_{fold}_test_data.csv', test_inputs, test_masks, test_labels)
