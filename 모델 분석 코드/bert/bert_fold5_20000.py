...
if not os.path.exists('./trained_model_12_20000/matrixs1'):
    os.makedirs('./trained_model_12_20000/matrixs1')
if not os.path.exists('./trained_model_12_20000/indexs1'):
    os.makedirs('./trained_model_12_20000/indexs1')
if not os.path.exists('./trained_model_12_20000/results1'):
    os.makedirs('./trained_model_12_20000/results1')
if not os.path.exists('./trained_model_12_20000/logs1'):
    os.makedirs('./trained_model_12_20000/logs1')
...
for train_index, test_index in kf.split(tokenized_data):
    ...
    # 훈련 및 테스트 데이터 분할
    train_inputs = [tokenized_data[i]['input_ids'].squeeze() for i in train_index]
    train_masks = [tokenized_data[i]['attention_mask'].squeeze() for i in train_index]
    train_labels = labels[train_index]

    test_inputs = [tokenized_data[i]['input_ids'].squeeze() for i in test_index]
    test_masks = [tokenized_data[i]['attention_mask'].squeeze() for i in test_index]
    ...
    training_args = TrainingArguments(
        output_dir=f'./trained_model_12_20000/results1/fold_{fold}', num_train_epochs=3, per_device_train_batch_size=4, per_device_eval_batch_size=8,
        gradient_accumulation_steps=8, gradient_checkpointing=False, warmup_steps=600, weight_decay=0.01,
        logging_dir=f'./trained_model_12_20000/logs1/fold_{fold}', logging_steps=10, evaluation_strategy="epoch", save_strategy="epoch", load_best_model_at_end=True,
    ...
    # 모델 학습
    trainer.train()
    model.save_pretrained(f'./trained_model_12_20000/models1/fold_{fold}')
    ...
    # 결과 저장 및 출력
    app_metrics_df.to_csv(f'./trained_model_12_20000/results1/fold_{fold}_application_metrics.csv', index=False)
    ...
    plt.tight_layout()
    plt.savefig(f'./trained_model_12_20000/matrixs1/fold_{fold}_confusion_matrix.png')
    plt.close()
    ...
    # 훈련, 검증, 테스트 데이터 저장
    save_data(f'./trained_model_12_20000/indexs1/fold_{fold}_train_data.csv', train_inputs, train_masks, train_labels)
    save_data(f'./trained_model_12_20000/indexs1/fold_{fold}_val_data.csv', val_inputs, val_masks, val_labels)
    save_data(f'./trained_model_12_20000/indexs1/fold_{fold}_test_data.csv', test_inputs, test_masks, test_labels)
